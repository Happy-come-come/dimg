...
module.exports = {
	...
	async execute(interaction) {
		if(interaction.options.getString('url').match(/^(https?:\/\/.*\.com\/.*\/status\/[0-9]{1,}|https?:\/\/.*pixiv\.net\/artworks\/[0-9]{1,}(\/\?.*)?|https?:\/\/misskey.io\/notes\/[\w]*)(?!.*(\ |\<|\>|\*|\||\;|\[|\]|\{|\}|\"|\'|\(|\)|\$|\`|\\|\!).*).*$/)){
			...
			try{
				switch(true){
					case /^https?:\/\/.*\.com\/.*\/status\/[0-9]{1,}/.test(interaction.options.getString('url')):
